  ...
  useEffect(() => {
    ...
    return (
        ...
        <div style={{backgroundColor:'#151719',padding:'1rem',margin:'1rem',textAlign:'center'}}>
          ...
          <div className="col-sm-5"><div><img width="75" height="75" src={context.host.image} alt={context.host.name+"Logo"}/> {context.host.name}</div></div>
          <div className="col-sm-2">
            <br/><div style={{padding:'auto',margin:'auto'}}><strong >V.S</strong></div></div>
          <div className="col-sm-5"><div>{context.guest.name}<img width="75" height="75" src={context.guest.image} alt={context.guest.name+"Logo"}/></div></div>
          </div>
